...
export default function RegisterPage() {
  ...
  const handleCreateAccount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Backend now sends OTP automatically in registerUser
      setStep(2);
          ...
          {step === 1 ? (
            ...
            <form onSubmit={handleVerifyOtp} className="space-y-6 animate-slide-up">
               <div className="space-y-1.5 text-center">
                  <label className="text-[10px] font-black uppercase tracking-[0.3em] text-[#861C1C] block mb-2">Account Activation Code</label>
                  <input
                    type="text"
                    maxLength={6}
                    value={form.otp}
                    ...
                    className="w-full text-center text-5xl font-black py-8 rounded-[40px] bg-[#ECCEB6]/30 border-2 border-transparent text-[#861C1C] tracking-[0.4em] outline-none transition-all focus:border-[#861C1C]/20 shadow-inner"
                    required
                  />
               </div>
               ...
               <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-5 rounded-2xl bg-[#861C1C] text-white font-black text-sm uppercase tracking-widest hover:bg-[#6b1616] transition-all transform hover:-translate-y-1 shadow-2xl shadow-[#861C1C]/20 disabled:opacity-50"
               >
                  {loading ? 'Authenticating...' : 'Confirm & Enter'}
               </button>

               <button 
                  type="button"
                  onClick={() => setStep(1)}
                  className="w-full text-center text-[10px] uppercase font-black tracking-widest text-[#2B1D1C]/40 hover:text-[#861C1C] transition-colors"
               >
